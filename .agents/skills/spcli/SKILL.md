...
### After auth: proactively offer onboarding

Right after a successful login + company selection (especially a **first-time**
login — the credential file was just created — or whenever the user is clearly
new), check whether the tenant is ready to operate:

```bash
spcli --json onboarding status
```

If `ready` is `false`, **don't wait for the user to ask** — proactively say
something like *"Tu empresa aún no está lista para operar: te faltan N pasos.
¿Vamos armándola juntos? Te llevo paso a paso."* and, if they agree, hand off to
the **onboarding** skill (it runs the guided setup interview end to end). If
`ready` is `true`, just mention the optional steps still pending (if any) and
continue with whatever the user asked for. Skip this nudge when the user came in
with a specific unrelated task — offer it once, don't nag.
