      ...
      <Card glass>
        <Show when={!accountsQuery.isLoading} fallback={
          ...
          >
            ...
            >
              {/* Desktop: table. Mobile (<md): stacked cards so nothing gets
                  cramped into a horizontal scroll. */}
                ...
                <For each={accountsQuery.data ?? []}>{(acc) => (
                  <div class="rounded-lg border border-border p-3">
                    <div class="flex items-start justify-between gap-2">
                      <p class="min-w-0 flex-1 font-medium text-foreground">{acc.name}</p>
                      <Show when={isAdmin()}>
                    ...
                    <div class="mt-2 flex flex-wrap items-center gap-2 text-xs">
                      <Badge tone={typeBadgeTone}>{accountTypeLabel(acc.account_type)}</Badge>
                      <Badge tone={boolBadgeTone(acc.is_active)}>{acc.is_active ? 'Activa' : 'Inactiva'}</Badge>
                      <span class="text-muted-foreground">{acc.currency}</span>
                    </div>
                  </div>
                )}</For>
              </div>
