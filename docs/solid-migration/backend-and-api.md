...
Generated by reading the current codebase (`src/`, `frontend/` — the existing
Leptos/Rust WASM SPA). The Leptos SPA is the reference implementation for
"what does the frontend actually call" — a SolidJS rewrite must hit the exact
same endpoints, same request/response shapes, same cookies. This doc is that
contract.
...
- **Path prefix:** `/v2` — the built SPA's static assets (`index.html`,
  JS/WASM bundle, CSS) are mounted with `nest_service("/v2", spa_service)`.
- **Directory served:** `frontend/dist` by default, overridable via the
  `SPA_DIST` env var.
- **Prefix stripping:** `nest_service` strips `/v2` before handing the path to
  `ServeDir`, so a request for `/v2/accounts` resolves against
  `frontend/dist/accounts` inside the service.
- **SPA fallback:** `ServeDir::new(&spa_dir).fallback(ServeFile::new(spa_index))`
  — any path under `/v2` that doesn't match a real static file (e.g. a
  client-side route like `/v2/accounts`, `/v2/admin/projects/64f.../edit`)
  falls back to serving `index.html`, so the client-side router can take over.
- **NOT a global fallback:** the SPA service is only mounted at `/v2`. Any
  unmatched path outside `/v2` (e.g. a typo'd root path) still 404s — old
  pre-SPA behavior is unchanged for everything else.
- **Middleware on `/v2`:** none. The static file service is mounted directly
...
- Comment in the code confirms intent: "The SPA calls absolute `/api/...`
  paths (not `/v2/api`)" — i.e. the SolidJS app must also call `/api/...`
  as absolute same-origin paths, not prefix them with `/v2`.
