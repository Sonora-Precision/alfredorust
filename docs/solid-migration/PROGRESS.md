...
## FASE C — Pasada de animaciones  (PARALELO por grupo; tras verificar paridad de B)  — ✅ hecha 2026-07-07 (single-pass, no sub-agentes)

- [x] Transiciones de ruta (`<Transition mode="outin">`) — `components/layout/AppShell.tsx`, ~160ms, CSS en `index.css` (`.page-*`).
- [ ] Filas de tabla enter/remove/reorder (`<TransitionGroup moveClass>`) — **skip intencional**: `TransitionGroup`'s `onChange` mide `getBoundingClientRect()` de TODA la lista en cada cambio (ver `node_modules/solid-transition-group/dist/index.js`), y Solid's `<For>` re-keya por identidad de objeto — como las páginas re-fetchean con objetos nuevos en cada poll (no hay ids estables pasados a `<For>`), envolver `TableBody` en `TransitionGroup` habría animado la tabla completa en cada refetch en vez de solo altas/bajas reales — exactamente el anti-patrón que `animation-strategy.md` §4/§5 marca como "skip/avoid". Arreglarlo requiere estabilidad de keys a nivel de página (fuera de alcance: "no refactorizar lógica de página"). Revisar si se retoma tras Fase D.
- [x] Modales/drawers — **vía CSS keyframes + Kobalte `data-expanded`/`data-closed`** en vez de `<Presence>`+`<Motion>`: la presencia de Kobalte (`solid-presence`) solo escucha `animationend`, no `transitionend`, así que se necesitan `@keyframes` reales (`components/ui/Modal.tsx` + `.modal-overlay`/`.modal-content` en `index.css`). Evita doble-manejo de la misma transición por dos librerías.
- [x] Toasts — mismo mecanismo (`data-opened`/`data-closed`, `@keyframes toast-in/out`) en `components/ui/Toast.tsx`. El "stack reflow" al remover un toast es el reflow instantáneo normal de flex-column (Kobalte no expone su `<For>` interno para envolverlo en `TransitionGroup`).
- [ ] Accordion sidebar — **skip**: `NavGroup` usa `<details>` nativo (ya tiene el chevron animado); animar la altura del contenido nativo de forma barata (sin JS de altura medida) requeriría `@starting-style`/`transition-behavior` sobre `display`, soporte de navegador aún muy nuevo/inconsistente — no vale el riesgo para esta pasada.
- [x] KPIs: count-up + stagger de cards (dashboard) — `pages/Dashboard.tsx` (`Motion.div` stagger 0/.06/.12/.18s) + `lib/motion.ts` (`createCountUp`, tween manual con rAF + ease-out-cubic, ya que solid-motionone no expone spring y `@motionone/dom` está pensado para propiedades DOM, no para un número plano).
- [x] Micro-interacciones — Button: `active:scale-[0.97]` (transform, CSS puro) sumado al `transition-colors` de hover que ya existía. Card: **skip intencional** (no se le agregó hover/lift genérico — `Card` se usa como contenedor estático en la mayoría de las 19 páginas; un hover-lift global habría insinuado affordance de "clicable" donde no la hay). Reveal-on-scroll (`inView`): no aplicado — no hay secciones largas/scrolleables que lo justifiquen todavía.
- [x] Guard global `prefers-reduced-motion` — `index.css` (`@media (prefers-reduced-motion: reduce)`, fuerza duraciones a ~0 en vez de remover la animación, para no romper la lógica de unmount basada en `animationend`/`transitionend`) + `lib/motion.ts` (`prefersReducedMotion()`, usado en `Dashboard.tsx`).
